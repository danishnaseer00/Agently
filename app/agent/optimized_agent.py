"""Optimized agent with context compression and 413 error handling.

Instead of LangChain's AgentExecutor (which appends every tool result to the
scratchpad unboundedly), this uses a custom loop that:

- Compresses each tool result to ~300 tokens before storing
- Replaces old compressed context with newer (no unbounded growth)
- Retries on 413 errors with *freshly rebuilt* context (fixed retry bug)
- Hard-caps iterations at 4
...
def _is_token_limit_error(exc: Exception) -> bool:
    """Check if the exception is a token-limit / 413 error."""
    err = str(exc).lower()
...
def _build_context_block(
    ...
    """Assemble the full prompt string from pieces."""
    parts = [system_msg, f"\n\n## User question\n{user_block}"]
...
def run_optimized_agent(
    ...
    """Run an agent loop that compresses tool outputs into a short summary.

    Key differences from LangChain's AgentExecutor:
      - Tool results are compressed to ~*compress_chars* chars (≈300 tokens)
      - Old compressed entries may be dropped if a 413 error occurs
    ...
    # ── compressed history (each entry = 1 short string) ──────────────
    compressed_history: list[str] = []
    ...
    for iteration in range(max_iterations):
        ...
        try:
            ...
            if not _is_token_limit_error(exc):
                raise  # non-token error → propagate

            print(f"[OptAgent] Token-limit error (413/TPM), reducing context…",
                  file=sys.stderr)

            # ── retry strategy 1: drop all but last 2 compressed steps ──
            ...
            # ── retry strategy 2: clear ALL history ─────────────────────
            if response is None:
            ...
            # ── retry strategy 3: graceful fallback ─────────────────────
            if response is None:
                lines = "\n".join(
...
def _compress(text: str, max_chars: int = 1200) -> str:
    """Keep the beginning and end of *text*, discarding the middle.

    Rough heuristic: beginning tends to contain the key facts, end tends to
    contain conclusions.  1200 chars ≈ 300 tokens.
    """
...
def _clean_response(text: str) -> str:
    ...
    import re
    text = re.sub(r"【[^】]*】", "", text)
    # Also clean up any double spaces or leading/trailing whitespace left behind
...
def _synthesize_final(
    ...
    """Ask the LLM one more time to write a final answer from the summaries."""
